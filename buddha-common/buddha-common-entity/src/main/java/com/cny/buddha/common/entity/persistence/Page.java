...
public class Page<T> {
	...
	private int pageSize; // 页面大小，设置为“-1”表示不进行分页（分页无效） cny_note 在Mybatis的拦截器里做初始值的注入
	...
	/**
	 * 构造方法
	 * @param request 传递 repage 参数，来记住页码，使用户能回到原来的分页状态
	 * @param response 用于设置 Cookie，记住页码
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if (StringUtils.isNumeric(no)){
			this.setCookie(response, "pageNo", no, "/",60*60*24);
			this.setPageNo(Integer.parseInt(no));
		}else if (request.getParameter("repage")!=null){//回到上次的页码位置
			no = this.getCookie(request,null, "pageNo", false);
			if (StringUtils.isNumeric(no)){
				this.setPageNo(Integer.parseInt(no));
			}
		}
		...
		if (StringUtils.isNumeric(size)){
			this.setCookie(response, "pageSize", size, "/",60*60*24);
			this.setPageSize(Integer.parseInt(size));
		}else if (request.getParameter("repage")!=null){
			size = this.getCookie(request,null, "pageSize", false);
			if (StringUtils.isNumeric(size)){
				this.setPageSize(Integer.parseInt(size));
			}
        ...
        if (StringUtils.isNotBlank(funcName)){
			this.setCookie(response, "funcName", funcName, "/",60*60*24);
            this.setFuncName(funcName);
        }else if (request.getParameter("repage")!=null){
			funcName = this.getCookie(request,null, "funcName", false);
			if (StringUtils.isNotBlank(funcName)){
                this.setFuncName(funcName);
            }
        }
...
//	}
	 ...
	 * @param path uri 路径
	 */
	public  void setCookie(HttpServletResponse response, String name, String value, String path, int maxAge) {
